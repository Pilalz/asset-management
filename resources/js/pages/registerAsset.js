    ...
    if ($('#registerAssetTable').length) {
        $('#registerAssetTable thead tr:eq(0) th').each(function (i) {
            ...
            if (i === 0 || i === 8) {
                ...
                $(cell).html('<input type="number" min="1" class="filter-input w-full p-2 mx-1 text-xs border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200 outline-none transition-colors shadow-sm" />');
            }
            else if (i === 7) {
                $(cell).html(
                    '<select class="filter-select w-auto p-2 mx-2 my-2 text-xs border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">' +
