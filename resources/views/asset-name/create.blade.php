...
@section('content')
    ...
    <div class="relative overflow-x-auto shadow-md py-5 px-6 rounded-lg m-5 bg-white dark:bg-gray-800">
        <form class="max-w mx-auto" action="{{ route('asset-name.store') }}" method="POST">
            @csrf
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select a Sub Class <span class="text-red-900 dark:text-red-400">*</span></label>
                <select name="sub_class_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option selected value="">Choose a Sub Class</option>
                    @foreach($assetsubclasses as $asset_sub_class)
                        <option value="{{ $asset_sub_class->id }}" {{ old('sub_class_id') == $asset_sub_class->id ? 'selected' : '' }}>
            ...
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Asset Grouping <span class="text-red-900 dark:text-red-400">*</span></label>
                <input type="text" name="grouping" value="{{ old('grouping') }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="" required />
                 @error('grouping')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="flex flex-col gap-2 sm:flex-row">
                ...
                <a href="{{ route('asset-name.index') }}" class="text-gray-900 bg-gray-200 hover:bg-gray-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200">Cancel</a>
            </div>
        </form>
    </div>
@endsection