...
@section('content')
    @push('styles')
        <style>
            ...
            #assetNameTable thead tr td:hover {
                ...
                <div>
                    ...
                    <div id="dropdownAction"
                        ...
                        <ul class="py-1 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownActionButton">
                            ...
                            <li>
                                <a href="{{ route('asset-name.export') }}" type="button"
                                    class="group flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-200 dark:hover:bg-gray-600 transition-colors">
                                    <svg class="w-4 h-4 mr-3 text-gray-400 group-hover:text-indigo-500 dark:text-gray-500 dark:group-hover:text-indigo-400 transition-colors"
                                        aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none"
                                        viewBox="0 0 24 24">
                                ...
                                <div class="mb-4">
                                    <div class="flex items-center justify-center w-full">
                                        <label id="an-file-upload-label"
                                            ...
                                            <div id="an-file-upload-placeholder" class="flex flex-col items-center justify-center pt-5 pb-6">
                                            ...
                                            <div id="an-file-upload-selected" class="hidden flex-col items-center justify-center pt-5 pb-6">
                                                <svg class="w-10 h-10 mb-3 text-green-500" aria-hidden="true"
                                                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2m-6 9 2 2 4-4" />
                                                </svg>
                                                <p id="an-file-name-display" class="mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300 text-center px-4 break-all"></p>
                                                <p class="text-xs text-green-500 dark:text-green-400">File selected — click to change</p>
                                            </div>
                                            <input id="an-excel-file-input" name="excel_file" type="file" accept=".xlsx, .xls" class="hidden"
                                                required />
                                        </label>
                                    </div>
                                    <script>
                                        document.getElementById('an-excel-file-input').addEventListener('change', function () {
                                            var fileName = this.files[0] ? this.files[0].name : null;
                                            if (fileName) {
                                                document.getElementById('an-file-upload-placeholder').classList.add('hidden');
                                                document.getElementById('an-file-upload-selected').classList.remove('hidden');
                                                document.getElementById('an-file-upload-selected').classList.add('flex');
                                                document.getElementById('an-file-name-display').textContent = fileName;
                                            } else {
                                                document.getElementById('an-file-upload-placeholder').classList.remove('hidden');
                                                document.getElementById('an-file-upload-selected').classList.add('hidden');
                                                document.getElementById('an-file-upload-selected').classList.remove('flex');
                                            }
                                        });
                                    </script>
        ...
        <div
            ...
            <table id="assetNameTable" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-100">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-white">
                    <tr>
                        <th scope="col" class="px-6 py-3 hover:bg-transparent">No</th>
                        <th scope="col" class="px-6 py-3 hover:bg-transparent">Asset Sub Class</th>
                        <th scope="col" class="px-6 py-3 hover:bg-transparent">Asset Name</th>
                        <th scope="col" class="px-6 py-3 hover:bg-transparent">Asset Grouping</th>
                        <th scope="col" class="px-6 py-3 hover:bg-transparent">Commercial Life</th>
                        <th scope="col" class="px-6 py-3 hover:bg-transparent">Fiscal Life</th>
                        @can('is-admin')
                            <th scope="col" class="px-6 py-3 hover:bg-transparent">Actions</th>
                        @else
                            <th scope="col" class="px-6 py-3 hover:bg-transparent"></th>
                        @endcan
                    </tr>
                    <tr id="filter-row"
                        class="bg-gray-50/50 dark:bg-gray-700/50 border-t border-slate-200 dark:border-gray-600">
                        <td class="px-4 py-2 hover:bg-transparent"></td>
                        <td class="px-4 py-2 hover:bg-transparent"></td>
                        <td class="px-4 py-2 hover:bg-transparent"></td>
                        <td class="px-4 py-2 hover:bg-transparent"></td>
                        <td class="px-4 py-2 hover:bg-transparent"></td>
