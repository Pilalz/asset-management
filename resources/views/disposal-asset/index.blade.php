...
@section('content')
    <div class="bg-white flex p-5 text-lg justify-between">
        ...
        <div class="flex">
            <a href="{{ route('disposal-asset.create') }}" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-sm text-sm px-5 py-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
    ...
    <div class="p-5">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white p-4">
            <table id="disposalAssetTable" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @forelse ($disposalassets as $disposal_asset)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4">{{ $disposal_asset->form_no }}</td>
                            <td class="px-6 py-4">{{ $disposal_asset->submit_date }}</td>
                            <td class="px-6 py-4">{{ $disposal_asset->department->name }}</td>
                            <td class="px-6 py-4">$ {{ number_format($disposal_asset->nbv, 0, '.', ',') }}</td>
                            <td class="px-6 py-4">$ {{ number_format($disposal_asset->esp, 0, '.', ',') }}</td>
                            <td class="px-6 py-4">{{ $disposal_asset->sequence == 1 ? 'Yes' : 'No' }}</td>
                            <td class="px-6 py-4">{{ $disposal_asset->detail_disposals_count == null ? 'Kosong' : $disposal_asset->detail_disposals_count }}</td>
                            <td>
                                @php
                                    $statusText = $disposal_asset->status;
                                    $statusClass = '';
                                ...
                                @endphp
                                <span class="{{ $statusClass }} text-xs font-medium px-2 py-0.5 rounded">{{ $disposal_asset->status }}</span>
                            </td>
                            <td>
                                <div class="flex">
                                    <a href="{{ route('disposal-asset.show', $disposal_asset->id) }}" type="button" class="text-white bg-gray-700 hover:bg-gray-800 font-medium rounded-lg text-xs px-3 py-2.5 text-center inline-flex items-center me-2 dark:bg-gray-600 dark:hover:bg-gray-700">
                                    ...
                                    @if ($disposal_asset->status === 'Approved')
                                    ...
                                    @else
                                        <a href="{{ route('disposal-asset.edit', $disposal_asset->id) }}" type="button" class="text-white bg-green-700 hover:bg-green-800 font-medium rounded-lg text-xs px-3 py-2.5 text-center inline-flex items-center me-2 dark:bg-green-600 dark:hover:bg-green-700">
                                            <svg class="w-3.5 h-3.5 mr-2 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 21">
                                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m14.304 4.844 2.852 2.852M7 7H4a1 1 0 0 0-1 1v10a1 1 0 0 0 1 1h11a1 1 0 0 0 1-1v-4.5m2.409-9.91a2.017 2.017 0 0 1 0 2.853l-6.844 6.844L8 14l.713-3.565 6.844-6.844a2.015 2.015 0 0 1 2.852 0Z"/>
                                            </svg>
                                            Edit
                                    ...
                                    <form action="{{ route('disposal-asset.destroy', $disposal_asset->id) }}" method="POST" class="">
                                        @csrf
                                        @method('DELETE')
...
<script>
    ...
    if (document.getElementById("disposalAssetTable") && typeof simpleDatatables.DataTable !== 'undefined') {
        const dataTable = new simpleDatatables.DataTable("#disposalAssetTable", {
            searchable: true,
            sortable: true,
        });
    }
