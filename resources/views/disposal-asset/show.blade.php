...
@section('content')
    ...
    <div class="bg-white flex p-5 text-lg justify-between dark:bg-gray-800">
        ...
        <div class="flex gap-2 content-center">
            <a href="{{ route('disposal-asset.exportPdf', $disposal_asset->id) }}" target="_blank"
                class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-sm text-sm px-5 py-2.5">
                Export PDF
            </a>
        </div>
    </div>
        ...
        <div class="relative overflow-x-auto shadow-md py-5 px-6 rounded-lg bg-white dark:bg-gray-800">
            ...
            <div class="grid grid-cols-1 gap-y-5 mb-5">
                ...
                <div>
                    ...
                    <p class="w-auto px-1 border-b border-gray-700 text-sm text-gray-900 dark:text-white">
                        {{ $disposal_asset->reason }}</p>
                </div>
                                    ...
                                    @foreach($disposal_asset->detailDisposals as $detail)
                                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 asset-row">
                                            <td
                                                ...
                                                {{ $loop->iteration }}</td>
                                            <td class="px-2 py-4">{{ $detail->asset->assetName->name }} -
                                                {{ $detail->asset->description }}</td>
                                            <td class="px-2 py-4">Rp {{ number_format($detail->kurs, 0, ',', '.') }}</td>
                                            @if($activeCompany->currency === 'USD')
                                                <td class="px-2 py-4">$ {{ number_format($detail->njab, 0, '.', ',') }}</td>
                                                <td class="px-2 py-4">Rp
                                                    {{ number_format(($detail->kurs * $detail->njab), 0, ',', '.') }}</td>
                                            @elseif($activeCompany->currency === 'IDR')
                                                <td class="px-2 py-4">Rp {{ number_format($detail->njab, 0, ',', '.') }}</td>
                                                <td class="px-2 py-4">$
                                                    {{ number_format(($detail->njab / $detail->kurs), 0, '.', ',') }}</td>
                                            @endif
                                        </tr>
                                    @endforeach
                                </tbody>
                    ...
                    <div class="border-2 border-black rounded-lg p-4 dark:border-gray-400">
                        ...
                        <div class="flex flex-row mb-2">
                            ...
                            <label class="w-auto mr-2 text-sm font-medium text-gray-900 dark:text-white">Sequence : <span
                                    class="{{ $sequenceClass }} font-medium px-2 py-0.5 rounded">{{ ($disposal_asset->sequence == 1) ? "Yes" : "No" }}</span></label>
                        </div>
                                            ...
                                            @if ($approv->status == 'approved' && $approv->user->signature)
                                                <td class="px-2 py-4 status-pending">
                                                    <div class="signature-container">
                                                        <img src="{{ $approv->user->signature }}" alt="Signature"
            ...
            <div class="flex gap-2 content-center">
                <a href="{{ route('disposal-asset.index') }}"
                    class="text-gray-900 bg-gray-200 hover:bg-gray-300 font-medium rounded-lg text-sm w-auto px-5 py-2.5 text-center dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600">Back</a>
                ...
                @if ($canApprove)
                        ...
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                <div class="p-4 md:p-5 text-center">
                                    ...
                                    <button id="confirm-approve-btn" data-modal-hide="confirmation-modal" type="button"
                    ...
                    <form id="approve-form" action="{{ route('disposal-asset.approve', $disposal_asset) }}" method="POST"
                        class="hidden">
                        @csrf
                    </form>
                @else
