        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                <li class="inline-flex items-center">
                    <a href="{{ route('location.index') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-indigo-600 dark:hover:text-indigo-400 dark:text-gray-400 transition-colors">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl border border-slate-200 dark:border-gray-700 overflow-hidden">
                ...
                <div class="px-6 py-5 border-b border-slate-200 dark:border-gray-700 bg-slate-50/50 dark:bg-gray-800/50">
                    ...
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Fill in the information below to add a new location to the system.
                    </p>
                </div>

                <!-- Form Section -->
                <form action="{{ route('location.store') }}" method="POST" class="px-6 py-6">
                    @csrf
                    <div class="space-y-6">
                        <!-- Input Name -->
                        <div>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4"
                                ...
                                placeholder="e.g. South Kalimantan Storage Facility">{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Optional: Provide additional details about this location.</p>
                        </div>
                    </div>
