    ...
    <div class="p-5">
        <div class="relative overflow-x-auto shadow-md py-5 px-6 sm:rounded-lg bg-white dark:bg-gray-900">
            <form class="max-w mx-auto" action="{{ route('person-in-charge.update', $personInCharge->id) }}" method="POST">
                ...
                <div class="mb-5">
                    <label for="position" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Position <span class="text-red-900">*</span></label>
                    <select id="position" name="position" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option value="User" {{ $personInCharge->position == 'User' ? 'selected' : '' }}>User</option>
                        <option value="User Manager" {{ $personInCharge->position == 'User Manager' ? 'selected' : '' }}>User Manager</option>
                        <option value="Site Director" {{ $personInCharge->position == 'Site Director' ? 'selected' : '' }}>Site Director</option>
                        <option value="Asset Management" {{ $personInCharge->position == 'Asset Management' ? 'selected' : '' }}>Asset Management</option>
                        <option value="CFO" {{ $personInCharge->position == 'CFO' ? 'selected' : '' }}>CFO</option>
                        <option value="Director" {{ $personInCharge->position == 'Director' ? 'selected' : '' }}>Director</option>
                    </select>
                    @error('position')
