...
@section('content')
    ...
    <div class="relative overflow-x-auto shadow-md py-5 px-6 rounded-lg m-5 bg-white dark:bg-gray-800">
        <form class="max-w mx-auto" action="{{ route('stock-opname.update', $stock_opname->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Title <span
                        class="text-red-900 dark:text-red-400">*</span></label>
                <input type="text" name="title" value="{{ old('title', $stock_opname->title) }}"
            ...
            <div class="mb-5">
                ...
                <input type="text" name="description" value="{{ old('description', $stock_opname->description) }}"
            ...
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status <span
                        class="text-red-900 dark:text-red-400">*</span></label>
                <select name="status" id="status" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
                    <option value="Open" {{ $stock_opname->status === 'Open' ? 'selected' : '' }}>Open</option>
                    <option value="Closed" {{ $stock_opname->status === 'Closed' ? 'selected' : '' }}>Closed</option>
                </select>
                @error('status')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-5">
                <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Start Date <span
                        class="text-red-900 dark:text-red-400">*</span></label>
                <input type="date" name="start_date" value="{{ old('start_date', $stock_opname->start_date->format('Y-m-d')) }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required />
                @error('start_date')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
